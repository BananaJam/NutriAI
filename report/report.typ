...
#report-template[
  ...
  *Чат* і пов'язані з ним повідомлення забезпечують збереження історії взаємодії користувача з AI-асистентом. Важливо, що повідомлення асистента можуть містити не лише текст, а й історію викликів інструментів та результати цих викликів.
  ...
  #figure(
    table(
      columns: (1.45fr, 1.2fr, 1.15fr, 1.15fr, 1.45fr),
      inset: 5pt,
      stroke: 0.6pt + rgb("#777777"),
      align: left + horizon,
      [*SDK*],
      [*Інтеграція в NutriAI*],
      [*Контроль за ходом виконання*],
      [*Спостережуваність*],
      [*Підсумкова оцінка придатності*],

      [Vercel AI SDK],
      [Найменший обсяг додаткового коду, оскільки базовий чат уже побудовано на близькому стеку],
      [Добрий контроль одноагентного tool loop без зайвої оркестрації],
      [Достатньо для product-сценарію, але слабше на рівні окремих агентних примітивів],
      [Найкращий базовий варіант для поточної архітектури NutriAI],

      [OpenAI Agents SDK],
      [Інтегрується швидко, якщо прийнятна OpenAI-орієнтована модель виконання],
      [Краще відображає агентні сутності, сесії та handoff-підходи],
      [Сильніші вбудовані можливості трасування та нативної агентної структури],
      [Найкращий кандидат для швидкого розширення без власної оркестрації],

      [LangGraph],
      [Потребує найбільшого інженерного каркаса та окремої адаптації],
      [Найявніший контроль графа, кроків і стану],
      [Зручний для складних workflow і довготривалої пам'яті],
      [Найбільш перспективний при ускладненні сценаріїв, але надлишковий для базового чату],
    ),
    caption: [Порівняння реалізованих SDK за інтеграційними властивостями в NutriAI.],
  )
